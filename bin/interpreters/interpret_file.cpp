...
void BF::interpret_file(const std::string &path, bool in_line) {
	...
	location = BF::find_parent(path);

	if (path[0] != '/') {
		location = (std::string)(std::filesystem::current_path()) + '/' + location;
	}

	BF::eval_string(input, pointer, arr, function_map, location, in_line);

	if (function_map["main"] == "") {
		std::cerr << "\nEmpty or no main definition!\n" << std::endl;
	}
	else {
		BF::eval_string(function_map["main"], pointer, arr, function_map, location, in_line);
	}
